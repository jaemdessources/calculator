...
calculator = {
  add: function (e) {
    //pour la gestion des boutons de l'interface.
    e.stopPropagation();
    if (/egal|del|ac|deg/.test(e.target.id) === true) {
      switch (e.target.id) {
        case "egal":
          ...
          break;
        case "del":
          for (i = 0, j = saisie.value.length; i <= j - 1; i++)
            text = i === j - 1 ? text : text + saisie.value[i];
      ...
      if (
        e.target.parentNode.className !== "functionButton" &&
        ...
        saisie.value =
          e.target.parentNode.className !== "functionButton"
            ? e.target.getAttribute("value")
            : onShift && e.target.parentNode.getAttribute("value1")
            ? e.target.parentNode.getAttribute("value1")
    ...
    if (/([*-/%+])/.test(saisie.value[saisie.value.length - 2]) && afterResult) {
      ...
      saisie.value = "Ans^";
    } else if (beforeCalcul && saisie.value !== "0") {
      text = "";
  ...
  hide: function () {
    ...
    let f = setInterval(function () {
      if (number >= -15) {
        ...
        document.getElementById("historique").style.width =
          document.getElementById("historique").style.width === "1.1%"
            ? number
            : number - 3 + "%";
  ...
  error: function (type, arg) {
    //gestion des operations a appliquer en cas d'erreur
    gv = false;
    switch (type) {
      case "syntaxe":
  ...
  inverser: function () {
    //gestion des deuxiemes calculator
    ...
    buttons = document.querySelectorAll(
      ".numberButton, .functionButton, .operatorButton"
    );
  },
  reverse: function () {
    onShift = onShift ? false : true;
  ...
  iemeCar: function (x, type) {
    //gestion de la suppression de caracteres
  ...
  evaluate: function (x, y) {
    // calculator d'evaluation, determine la valeur d'une expression
    number = null;
  ...
  calcul: function (x) {
    // ensemble de calculator qui calculent le resultat de l'operation
    if (x) {
      if (
        !/\u221a|\u00aa|\u03c0|\u002d|\u00b9|\u00b3|\u00b0|\u00b2|[!^]|[a-z]|\u2147/i.test(
          saisie.value
        ...
        //calculator.evaluate(this.forme);
  ...
  calculn: function (x) {
    ...
    if (
      ...
      /*  if (/sin\u002d*\u00b9*\(|cos\u002d*\u00b9*\(|tan\u002d*\u00b9*\(/i.test(facteurs[tabn][0].forme) === true) facteurs[tabn][0].forme = calculator.iemeCar(facteurs[tabn][0].forme, "b");*/
